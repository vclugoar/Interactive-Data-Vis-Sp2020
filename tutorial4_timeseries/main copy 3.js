...
function init() {
  ...
  xScale = d3
    .scaleLinear()
  //  .domain([0, d3.max(state.data, d => d.year)])
    .domain(d3.extent(state.data, d => d.week))
...
const areaFunc = d3
 ...
 const dot = svg
  .selectAll(".dot")
  .data(filteredData, d => d.week) // use `d.year` as the `key` to match between HTML and data elements
  .join(
    enter =>
