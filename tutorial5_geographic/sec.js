...
Promise.all([
  ...
  d3.csv("../data/Starbucks.csv", d3.autoType),
]).then(([geojson, starbucks]) => {
...
function init() {
  ...
  .enter()
  .append("svg:image")
  .attr("xlink:href", "../data/miniStarbucks.png")
  .attr("transform", d => {
    const [x, y] = projection([d.Long, d.Lat]);
    return `translate(${x}, ${y})`;
  })
  .on("mouseover", d => { 
    state.hover["Specific Location"] = d.Location;
  });
